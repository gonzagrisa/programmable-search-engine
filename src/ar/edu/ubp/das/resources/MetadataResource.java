...
public class MetadataResource {
	...
	public Response ping() {
		this.logger.log(MyLogger.INFO, "Petici贸n de ping exitosa");
	...
	public Response getMetadata() {
		try {
			...
			this.logger.log(MyLogger.INFO, "Petici贸n de metadatos exitosa");
			return Response.status(Status.OK).entity(metadata).build();
		} catch (Exception e) {
			this.logger.log(MyLogger.ERROR, "Petici贸n de metadatos con error: " + e.getMessage());
	...
	public Response deleteSelected(List<MetadataBean> metadataList) {
		if (metadataList.size() == 0) {
			return Response.status(Status.BAD_REQUEST).entity("La petici髇 no rellena los requisitos").build();
		}
		try {
			MetadataDao elastic = new MetadataDaoImpl();
	...
	public Response updateMetadata(MetadataBean metadata) {
		try {
			...
			this.logger.log(MyLogger.INFO, "Actualizaci贸n de metadato #" + metadata.getId() + " exitosa");
			return Response.status(Status.NO_CONTENT).build();
		} catch (Exception e) {
			this.logger.log(
				MyLogger.ERROR,
				"Eliminaci贸n de metadato #" + metadata.getId() + " con error: " + e.getMessage()
			);
			return Response.status(Status.BAD_REQUEST).entity(e.getMessage()).build();
		}
	}
